...
function sliderToCapital(v) {
  // v: 0–100 → Rp 1.000.000 – Rp 1.000.000.000
...
function renderCapitalData(data) {
  ...
  // CVaR: use portfolio vol as proxy (approximate daily CVaR = vol * 1.65 / sqrt(252))
  const pvol    = data.portfolio.vol / 100;
  const dailyCV = pvol / Math.sqrt(252) * 2.33;   // 99% approx
...
function render(data) {
  ...
  advEl.classList.remove("skeleton"); advEl.style.height = advEl.style.width = "";
  advEl.textContent = data.advice;

  // News
  document.getElementById("news-list").innerHTML = data.news.map(n => {
    const cls = n.badge==="NEG"?"#FDEAEA:#C0392B":"NEG"? "" :
                n.badge==="POS"?"#E8F8F0:#1A6B3A":"#F0EDE6:#888780";
    const [bg,col] = n.badge==="NEG" ? ["#FDEAEA","#B03A2E"] :
                     n.badge==="POS" ? ["#E4F4EC","#1A6B3A"] : ["var(--surface2)","var(--text3)"];
  ...
  // Quant panel (populate but don't show)
...
function renderQuantPanel(data) {
  ...
  // Signal bars — gunakan sub_scores dari server
  ...
  // GARCH chart — pakai data nyata dari server
...
function buildGarchChart(garchSeries, portVol) {
  ...
  // Gunakan data nyata dari server jika tersedia, fallback ke simulasi
  let vols;
  ...
  garchChart = new Chart(ctx, {
    ...
    data:{
      labels,
      datasets:[{
        data:vols, borderColor:"#2980B9", borderWidth:1.5, fill:true,
        backgroundColor:"rgba(41,128,185,0.08)", tension:.3, pointRadius:0,
      }],
    },
    options:{
...
const FALLBACK = {
  crisis: {
    ...
    signals:[{name:"Volatilitas pasar",val:"Sangat tinggi",score:0,color:"#E24B4A"},{name:"Sentimen berita",val:"Sangat negatif",score:9,color:"#E24B4A"},{name:"Tren harga",val:"Turun tajam",score:20,color:"#EF9F27"},{name:"Diversifikasi",val:"Melemah",score:35,color:"#EF9F27"}],
  ...
  bearish: {
    ...
    signals:[{name:"Volatilitas pasar",val:"Tinggi",score:0,color:"#E24B4A"},{name:"Sentimen berita",val:"Negatif",score:29,color:"#EF9F27"},{name:"Tren harga",val:"Sideways",score:50,color:"#888780"},{name:"Diversifikasi",val:"Normal",score:80,color:"#639922"}],
  ...
  moderate: {
    ...
    signals:[{name:"Volatilitas pasar",val:"Sedang",score:45,color:"#EF9F27"},{name:"Sentimen berita",val:"Netral",score:52,color:"#888780"},{name:"Tren harga",val:"Sideways",score:50,color:"#888780"},{name:"Diversifikasi",val:"Baik",score:80,color:"#639922"}],
  ...
  bullish: {
    ...
    signals:[{name:"Volatilitas pasar",val:"Rendah",score:72,color:"#639922"},{name:"Sentimen berita",val:"Positif",score:74,color:"#639922"},{name:"Tren harga",val:"Naik",score:67,color:"#639922"},{name:"Diversifikasi",val:"Baik",score:80,color:"#639922"}],
  ...
  euphoria: {
    ...
    signals:[{name:"Volatilitas pasar",val:"Sangat rendah",score:90,color:"#639922"},{name:"Sentimen berita",val:"Sangat positif",score:89,color:"#639922"},{name:"Tren harga",val:"Naik kuat",score:83,color:"#639922"},{name:"Diversifikasi",val:"Melemah",score:35,color:"#EF9F27"}],
...
async function fetchData(scenario) {
  try {
    const res  = await fetch(`${API}/api/analyze?scenario=${scenario}`, { signal: AbortSignal.timeout(3000) });
    ...
    if (!isOnline) {
      ...
      document.getElementById("conn-label").textContent = "Terhubung — Live";
...
// Update keyword & SSE status
function updateStatusBar(data) {
  const kwEl  = document.getElementById('kw-indicator');
  const sseEl = document.getElementById('sse-indicator');
  if (kwEl) kwEl.textContent = 'Keywords: ' + (data.sentiment_meta && data.sentiment_meta.n_articles ? data.sentiment_meta.n_articles + ' art.' : '—');
  if (sseEl) {
    const connected = _evtSource && _evtSource.readyState === EventSource.OPEN;
    sseEl.textContent = connected ? '● SSE Live' : '○ SSE Off';
    sseEl.style.color = connected ? 'var(--ok,#1A6B3A)' : 'var(--text-tertiary,#888)';
  }
}
...
async function loadBacktest() {
  if (_btData) { renderBacktest(_btData); return; }
  try {
    const r = await fetch(API+'/api/backtest', {signal:AbortSignal.timeout(9000)});
...
function renderBacktest(d) {
  ...
  // Out-of-sample label (lebih credible)
  ...
  set('bt-note2',
    'Simulasi 5 tahun: IHSG '+(s.ihsg_total_return>0?'+':'')+s.ihsg_total_return+'% vs D.A.R.A '+(s.dara_total_return>0?'+':'')+s.dara_total_return+'%. '+
...
// Patch switchTab to load backtest
...
// Auto-load backtest stats for banner
setTimeout(() => {
    ...
    .then(r=>r.json()).then(d=>{
      if (d.stats) {
        ...
        ['bt-vol-reduction','bt-vol-r2'].forEach(id=>{
          const el=document.getElementById(id); if(el) el.textContent=(s.oos_vol_reduction!=null ? Math.round(s.oos_vol_reduction) : Math.round(s.vol_reduction_pct))+'%';
        });
        ['bt-crisis-protect','bt-crisis-p2'].forEach(id=>{
          const el=document.getElementById(id); if(el) el.textContent='+'+s.crisis_protection.toFixed(1)+'pp';
        });
      }
    }).catch(()=>{});
}, 1500);
  ...
  try {
    ...
    _evtSource.onmessage = (e) => {
      try {
        ...
        _reconnectDelay = 3000; // reset on success
      } catch (err) {
    ...
    // Browser tidak support EventSource atau error lain
...
// Wrapper yang dipakai oleh setGoal() dll
async function fetchData(scenario) {
